...
#[derive(Debug, Clone)]
pub enum SwarmMpscTxFlag {
    NewStream,
    Connect,
    Disconnect,
}

impl SwarmMpscTxFlag {
    pub fn tag(&self) -> u8 {
        match *self {
            Self::NewStream => 1,
            Self::Connect => 2,
...
pub fn process_host_frame(payload: Vec<u8>) -> Result<(SwarmMpscTxFlag, HeaderValue)> {
    let mut idx = 0;

    let tag = payload[idx];
    idx += 1;

    let flag = SwarmMpscTxFlag::from_tag(tag)
...
mod tests {
    ...
    fn test_roundtrip_with_vec() {
        ...
        assert!(matches!(decoded_flag, SwarmMpscTxFlag::Connect));
